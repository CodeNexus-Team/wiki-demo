...
def _build_resume_user_prompt(
    ...
    allow_edit: 是否启用修改意图注入,由调用方根据 _should_allow_suggest_edit(user_query) 决定。
    """
...
async def run_detailed_query(
    ...
    if resume_session_id:
        ...
        # 判断两种注入触发条件:
        # 1. 跨页追问 → 注入新页面的 outline / block_inventory / source_context
        # 2. 修改意图 → 额外注入修改协议段(补齐 session 血统缺失的修改能力)
        last_page = _session_last_page.get(resume_session_id)
        is_cross_page = last_page is not None and last_page != page_path
        is_edit_intent = _should_allow_suggest_edit(user_query)
        need_inject = is_cross_page or is_edit_intent

        if need_inject:
            reasons = []
            if is_cross_page:
                reasons.append(f"跨页 {last_page}→{page_path}")
            if is_edit_intent:
                reasons.append("含修改信号")
            agent_logger.info(f"[Detailed resume] 注入新上下文: {', '.join(reasons)}")
            prompt_text = _build_resume_user_prompt(
                user_query, page_path, wiki_root, allow_edit=is_edit_intent
            )
        else:
...
async def run_qa_query(
    ...
    if resume_session_id:
        ...
        if need_inject:
            ...
            if is_edit_intent:
                reasons.append("含修改信号")
            agent_logger.info(f"[QA resume] 注入新上下文: {', '.join(reasons)}")
            prompt_text = _build_resume_user_prompt(
                user_query, page_path, wiki_root, allow_edit=is_edit_intent
            )
        else:

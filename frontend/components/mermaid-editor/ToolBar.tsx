...
import { FlowDirection, supportsDirection, supportsSpacing } from '../../hooks/useMermaidEditor';
...
export const ToolBar: React.FC<ToolBarProps> = ({
  ...
  const hasDirection = supportsDirection(chartType);
  const hasSpacing = supportsSpacing(chartType);

  // 没有任何可用控件时不渲染
  if (!hasDirection && !hasSpacing && !onResetLayout) return null;

  return (
    <div className="flex flex-col gap-4 p-4 bg-white/80 backdrop-blur-xl border-b border-gray-200/60">
      {/* 方向控制 — 仅 flowchart/graph */}
      {hasDirection && (
      ...
      {/* 不支持任何布局控制时的提示 */}
      {!hasDirection && !hasSpacing && (
        <div className="text-xs text-gray-400 italic">
          {chartType} 类型图表不支持方向和间距调整
        </div>
      )}

      {/* 操作按钮 */}
      {onResetLayout && hasDirection && (
